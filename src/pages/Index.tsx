...
const Index = () => {
  ...
  const gridCols = activeStats.length === 6 ? "lg:grid-cols-6" :
    activeStats.length === 5 ? "lg:grid-cols-5" :
      activeStats.length === 4 ? "lg:grid-cols-4" :
        activeStats.length === 3 ? "lg:grid-cols-3" : "lg:grid-cols-2";
  ...
  return (
    <>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-8 border-t border-white/10 pt-10 max-w-xl">
              {[
                ...
                { label: "Monthly Units", value: activeStats.find(s => s.id === "capacity")?.value || "20k+" },
                { label: "Export Countries", value: activeStats.find(s => s.id === "export")?.value || "15+" },
              ].map((stat, i) => (
                <motion.div
                  key={stat.label}
                  initial={{ opacity: 0, y: 20 }}
      ...
      <Section className="py-24 bg-white border-b border-secondary/20">
        <div className={cn("grid grid-cols-2 gap-0 divide-x divide-secondary/20 border-x border-secondary/20 px-6 lg:px-12 container mx-auto", gridCols)}>
          {activeStats.map((item: any, i: number) => (
            <motion.div
              key={item.id}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.1 }}
              className="py-12 px-8 flex flex-col items-center text-center group hover:bg-secondary/5 transition-colors"
            >
              {item.id === "experience" && <Award className="text-accent mb-4 group-hover:scale-110 transition-transform" size={24} />}
              {item.id === "export" && <Globe className="text-accent mb-4 group-hover:scale-110 transition-transform" size={24} />}
              {item.id === "capacity" && <BarChart3 className="text-accent mb-4 group-hover:scale-110 transition-transform" size={24} />}
              {item.id === "craftsmen" && <Users className="text-accent mb-4 group-hover:scale-110 transition-transform" size={24} />}
              {item.id === "factory" && <Factory className="text-accent mb-4 group-hover:scale-110 transition-transform" size={24} />}
              {item.id === "compliance" && <ShieldCheck className="text-accent mb-4 group-hover:scale-110 transition-transform" size={24} />}

              <p className="font-heading text-2xl font-bold text-charcoal">{item.value}</p>
              <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">{item.label}</p>
              <p className="text-[10px] text-muted-foreground/60 uppercase mt-2">{item.description || item.sub}</p>
            </motion.div>
          ))}
        </div>
      </Section>
